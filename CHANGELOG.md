...
- v1.59.0: Comprehensive polish and refinement pass — full codebase audit with 6 parallel analysis agents. Fix production CSP blocking Google Fonts (broken typography). Fix health endpoint returning HTTP 200 when database is down (now returns 503). Fix graceful shutdown race (process.exit before server.close callback). Fix authService re-lock bug where stale failed_login_attempts caused immediate re-lockout after expiry. Fix UsersTab self-role guard using wrong field name (userId vs id). Fix JWT base64url decoding. Fix logger meta overwrite allowing log poisoning. Fix anthropicService hardcoded timeout message and overly broad circuit breaker detection. Fix migration 007 unsafe ELSE role passthrough and fragile INSERT INTO...SELECT FROM pattern. Fix ProcessingView showing spinner on inactive Complete step. Fix StyleGuideModal double-close race condition. Fix duplicate Tailwind surface-200 color value. Add Vite dev server proxy for API routes. Add SPA fallback cache-control headers. Fix DEPLOYMENT.md manual secret lengths to match deploy.sh. Fix stale test comments referencing old role names. Remove dead code (categorization.js unreachable block, generation.js unused children field, unused barrel export, unused isUserLimited). Add try-catch to auth/me route handler. Update database.js JSDoc for case-sensitivity accuracy. Fix comments.js non-deterministic toLocaleString. All 648 tests pass (522 backend + 126 frontend).
...
- v1.58.0: Color palette overhaul — assimilate new muted design tokens into Tailwind config (brand, surface, teal, amber, rose, mauve scales), update all 22 component files with new color classes, rename info-box-blue→info-box-teal and info-box-red→info-box-rose CSS utilities, update role tag colors (admin=mauve, user=teal, guest=slate), refresh glass morphism system and button tokens with softer, more desaturated values.
