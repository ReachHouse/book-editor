...
SQLite with 9 migrations in `database/migrations/` (001-009). Migrations run automatically on server startup via `services/database.js`. Tables: users, sessions, invite_codes, usage_logs, projects, role_defaults, schema_version. No manual migration steps needed.
...
| Using `ELSE role END` in migration CASE | Never pass through unknown role values. Always explicitly map every valid role and use a safe fallback like `ELSE 'user'`. The VPS had `viewer` in role_defaults (not `restricted`) — unexpected data caused a CHECK constraint failure that took 4 deploy attempts to diagnose. |
...
| Running `ALTER TABLE RENAME` with `foreign_keys = ON` | If `PRAGMA foreign_keys = ON` when you rename a table, SQLite rewrites FK references in all dependent tables to the new name. If the migration then fails and rolls back, those FK rewrites may persist — leaving stale references to a table that no longer exists. The migration runner disables FK during migrations to prevent this; see migration 009 for the repair migration that fixed this on the VPS. |
